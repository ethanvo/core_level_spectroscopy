...
materials = ["c", "si", "sic", "bn", "aln", "alp", "mgo"]
...
def check_keydict(formula, basis, kdensity, orbital, frozen, fout):
    key = "{}_{}_k{}_{}".format(formula, basis, kdensity, orbital)
    if frozen is not None:
        key += "_nvir_act{}".format(int(res.root))
    filename = "data/{}_cbe.json".format(key)
                ...
                for kmax in range(4, 1, -1):
                    ...
                    if frozen is not None:
                        for kdensity in range(kmax, 1, -1):
                            check_keydict(formula, basis, kdensity, orbital, frozen, fout)
                    else:
                        kdensity = kmax
                        check_keydict(formula, basis, kdensity, orbital, frozen, fout)
