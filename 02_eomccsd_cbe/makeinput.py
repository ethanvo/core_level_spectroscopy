...
def get_keydict(
    ...
    material = dict(
        [
            ...
            ("nocc", nocc),
            ("nvir", nvir),
            ("nvir_act", int(res.root) if frozen is not None else None),
            ("frozen_core", frozen_core[formula]),
            ("frozen", frozen),
            ("chk", "{}.chk".format(key)),
            ("cderi", "{}_cderi.h5".format(key)),
            ("ekrhf", "{}_ekrhf.json".format(key)),
            ("ekrccsd", "{}_ekrccsd.json".format(key)),
    ...
    fout.write(f"sbatch -J {key} /burg/berkelbach/users/eav2136/builds/work_tools/slurm/pyscf/pyscf-job.sh base.py {key}.json\n")
    dump(material, filename)
    return key, material, filename


with open("sendall.sh", "w") as fout:
            ...
            for orbital, core in core_orbitals[formula].items():
                for kmax in range(4, 1, -1):
                    ...
                    if (
                        ...
                        frozen = frozen_core[formula] + list(
                            range(nocc + int(res.root), nmo)
                        )
                    if frozen is not None:
                        for kdensity in range(kmax, 1, -1):
                            key, material, filename = get_keydict(
                                formula,
                                basis,
                                kdensity,
                                orbital,
